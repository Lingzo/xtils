...
class StringView {
 public:
  ...
  StringView(const T& x) : StringView(x.data, x.size) {
    DCHECK(x.size == 0 || x.data != nullptr);
  }
  ...
  StringView substr(size_t pos, size_t count = npos) const {
    if (pos >= size_) return StringView("", 0);
...
inline bool operator<(const StringView& x, const StringView& y) {
  ...
  if (size == 0) return x.size() < y.size();
...
template <>
struct std::hash<::xtils::StringView> {
  size_t operator()(const ::xtils::StringView& sv) const {
