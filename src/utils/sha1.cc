...
inline uint32_t S(uint32_t n, uint32_t X) { return (X << n) | (X >> (32 - n)); }
...
void SecureHashAlgorithm::Final() {
  ...
  for (size_t t = 0; t < 5; ++t) H[t] = BSwap32(H[t]);
}
...
void SecureHashAlgorithm::Process() {
  ...
  // memcpy(W, M, sizeof(M));
  for (t = 0; t < 16; ++t) W[t] = BSwap32(W[t]);
...
void SecureHashAlgorithm::Pad() {
  ...
  if (cursor > 64 - 8) {
    ...
    while (cursor < 64) M[cursor++] = 0;

    Process();
  }

  while (cursor < 64 - 8) M[cursor++] = 0;
