...
void number_type_test() {
  ...
  // 测试整数解析
  auto int_json = xtils::Json::parse(R"({"value": 42})");
  ...
  // 测试浮点数解析
  auto float_json = xtils::Json::parse(R"({"value": 3.14})");
  ...
  // 测试科学计数法（应为浮点）
  auto sci_json = xtils::Json::parse(R"({"value": 1e5})");
  ...
  // 测试精度保持
  auto precision_json = xtils::Json::parse(R"({"value": 9007199254740993})");
  ...
  // 测试可选访问方法
  auto test_json = xtils::Json::parse(R"({"int": 42, "float": 3.14})");
  if (test_json) {
  ...
  // 测试序列化
  xtils::Json int_obj(42);
  xtils::Json float_obj(3.14);
...
void optional_access_test() {
  ...
  auto json = xtils::Json::parse(json_str);
  if (!json) {
  ...
  if (tags.has_value() && tags.value().size() == 2) {
    xtils::Json tags_json(tags.value());
...
void stress_test() {
  ...
  auto deep_json = xtils::Json::parse(deep_array);
  if (deep_json) {
    ...
    auto shallow_json = xtils::Json::parse(shallow_array);
    if (shallow_json) {
  ...
  auto large_json = xtils::Json::parse(large_object);
  if (large_json) {
  ...
  auto unicode_json = xtils::Json::parse(unicode_stress);
  if (unicode_json) {
...
void boundary_test() {
  ...
  auto max_depth_json = xtils::Json::parse(max_depth);
  if (max_depth_json) {
  ...
  auto over_max_json = xtils::Json::parse(over_max_depth);
  if (!over_max_json) {
...
int main(int argc, char* argv[]) {
  ...
  for (size_t i = 0; i < invalid_json_test_cases.size(); i++) {
    ...
    auto json = xtils::Json::parse(str);
    if (json) {
      // 只显示前100个字符以避免输出过长
  ...
  for (size_t i = 0; i < valid_json_test_cases.size(); i++) {
    ...
    if (!json) {
      ...
      try {
        ...
        auto reparsed = xtils::Json::parse(dumped);
        if (!reparsed) {
          std::string display_str =
              str.length() > 100 ? str.substr(0, 100) + "..." : str;
